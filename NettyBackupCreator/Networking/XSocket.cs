...
namespace NettyBackupCreator.Networking
{
    ...
    public class XSocket
    {
        ...
        public XSocket(string address, int port) : this()
        {
            ...
            _address = Regex.IsMatch(address, @"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$")
        ...
        {
            if (!_socket.IsBound && !_socket.Connected)
                throw new Exception("Unable to read. Socket is not bound or connected.");

            _readBuffer = new byte[BufferSize];
            try
            {
                if (usingStreams)
                    _socket.BeginReceive(_readBuffer, 0, _readBuffer.Length, SocketFlags.None, OnReceiveBufferedCallback,
                        _socket);
                else
                    _socket.BeginReceive(_readBuffer, 0, _readBuffer.Length, SocketFlags.None, OnReceiveCallback,
                        _socket);
            }
            catch (Exception e)
            {

...
namespace NettyBaseReloaded.Game.netty.packet
{
    class Builder
    {
        ...
        private void SendSlotbars(GameSession gameSession)
        {
            var player = gameSession.Player;
            if (player.UsingNewClient)
            {
                var slotbarItemsTest = new List<commands.new_client.SlotbarQuickslotItem>
                {
                    new commands.new_client.SlotbarQuickslotItem(1, "ammunition_laser_ucb-100"),
                    new commands.new_client.SlotbarQuickslotItem(2, "ammunition_laser_rsb-75"),
                    new commands.new_client.SlotbarQuickslotItem(3, "ammunition_laser_mcb-50"),
                    new commands.new_client.SlotbarQuickslotItem(4, "ammunition_laser_ucb-100"),
                    new commands.new_client.SlotbarQuickslotItem(5, "ammunition_laser_sab-50"),
                    new commands.new_client.SlotbarQuickslotItem(6, "ammunition_mine_smb-01"),
                    new commands.new_client.SlotbarQuickslotItem(7, "equipment_extra_cpu_ish-01"),
                    new commands.new_client.SlotbarQuickslotItem(8, "ammunition_specialammo_emp-01"),
                };

                var premium = new List<commands.new_client.SlotbarQuickslotItem>
                {
                    new commands.new_client.SlotbarQuickslotItem(1, "drone_formation_f-11-he"),
                    new commands.new_client.SlotbarQuickslotItem(2, "drone_formation_f-10-cr"),
                    new commands.new_client.SlotbarQuickslotItem(3, "drone_formation_f-09-mo")
                };


                var slotbars = new List<commands.new_client.SlotbarQuickslotModule>
                {
                    new commands.new_client.SlotbarQuickslotModule("standardSlotBar", slotbarItemsTest, "50,85|0,40",
                        "0", true),
                    new commands.new_client.SlotbarQuickslotModule("premiumSlotBar", premium, "50,85|0,80", "0", true)
                };
        ...
        public void StationCreateCommand(GameSession gameSession, Station station)
        {
            if (gameSession.Player.UsingNewClient)
            {
                ...
                {
                    foreach (var stationAsset in station.ToAssets(gameSession.Player.Spacemap))
                    {
                        AssetCreateCommand(gameSession, stationAsset);
                    }
                }
            }
            else

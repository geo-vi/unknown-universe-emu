...
using AmmunitionTypeModule = Server.Game.netty.commands.old_client.AmmunitionTypeModule;
using AssetTypeModule = Server.Game.netty.commands.new_client.AssetTypeModule;
using AttackMissedCommand = Server.Game.netty.commands.new_client.AttackMissedCommand;
using AttackTypeModule = Server.Game.netty.commands.new_client.AttackTypeModule;
using AudioSettingsModule = Server.Game.netty.commands.new_client.AudioSettingsModule;
using ClanRelationModule = Server.Game.netty.commands.new_client.ClanRelationModule;
using DisplaySettingsModule = Server.Game.netty.commands.new_client.DisplaySettingsModule;
using GameplaySettingsModule = Server.Game.netty.commands.new_client.GameplaySettingsModule;
using KillScreenOptionModule = Server.Game.netty.commands.old_client.KillScreenOptionModule;
using KillScreenPostCommand = Server.Game.netty.commands.old_client.KillScreenPostCommand;
using Object = NettyBaseReloaded.Game.objects.world.map.Object;
using OreTypeModule = Server.Game.netty.commands.new_client.OreTypeModule;
using PetGearTypeModule = Server.Game.netty.commands.old_client.PetGearTypeModule;
using POIDesignModule = Server.Game.netty.commands.new_client.POIDesignModule;
using POITypeModule = Server.Game.netty.commands.new_client.POITypeModule;
using QualitySettingsModule = Server.Game.netty.commands.new_client.QualitySettingsModule;
using ShipWarpModule = Server.Game.netty.commands.new_client.ShipWarpModule;
using ShipWarpWindowCommand = Server.Game.netty.commands.new_client.ShipWarpWindowCommand;
using UserKeyBindingsUpdate = Server.Game.netty.commands.new_client.UserKeyBindingsUpdate;
using VisualModifierCommand = Server.Game.netty.commands.new_client.VisualModifierCommand;
using WindowSettingsModule = Server.Game.netty.commands.new_client.WindowSettingsModule;
    ...
    class Builder
    {
        ...
        public void UserSettingsCommand(GameSession gameSession)
        {
            ...
            if (player.UsingNewClient)
            {
                ...
                var ws = new WindowSettingsModule(8,
                    "23,1|24,1|25,1|27,1|26,1|100,1|34,1|36,1|33,1|35,1|39,1|38,1|37,1|32,1|", false);
                var gm = new GameplaySettingsModule(false, false, false, false, false, true, false,
                    false, false, false);
                var z9 = new QuestSettingsModule(false, true, true, false, false, false);
                var ds = new DisplaySettingsModule(true, true, true, true, true, true, true, true,
        ...
        private void SendWindows(GameSession gameSession)
        {
            ...
            var slotbars = new List<commandKn>
            {
                new commandKn(commandKn.GAME_FEATURE_BAR, topLeftButtons, "0,0",
                    "0"),
                new commandKn(commandKn.GENERIC_FEATURE_BAR, topRightButtons,
                    "100,0", "0")
            };

            gameSession.Client.Send(WindowsCommand.write(slotbars).Bytes);
        ...
        public void SendSlotbars(GameSession gameSession)
        {
            ...
            {
                var ammo = new List<AmmunitionCountModule>();
                ammo.Add(new AmmunitionCountModule(
                    new AmmunitionTypeModule(
                        AmmunitionTypeModule.X1),
                    player.Information.Ammunitions["ammunition_laser_lcb-10"].Get()));
                ammo.Add(new AmmunitionCountModule(
                    new AmmunitionTypeModule(
                        AmmunitionTypeModule.X2),
                    player.Information.Ammunitions["ammunition_laser_mcb-25"].Get()));
                ammo.Add(new AmmunitionCountModule(
                    new AmmunitionTypeModule(
                        AmmunitionTypeModule.X3),
                    player.Information.Ammunitions["ammunition_laser_mcb-50"].Get()));
                ammo.Add(new AmmunitionCountModule(
                    new AmmunitionTypeModule(
                        AmmunitionTypeModule.X4),
                    player.Information.Ammunitions["ammunition_laser_ucb-100"].Get()));
                ammo.Add(new AmmunitionCountModule(
                    new AmmunitionTypeModule(AmmunitionTypeModule
                ...
                gameSession.Client.Send(commands.old_client.AmmunitionCountUpdateCommand.write(ammo).Bytes);
                ammo.Clear();
                ammo.Add(new AmmunitionCountModule(
                    new AmmunitionTypeModule(AmmunitionTypeModule
            ...
            if (gameSession.Player.UsingNewClient)
            {
                if (character is Player)
                {
                    ...
                    bytes = commands.new_client.ShipCreateCommand.write(pChar.Id,
                        ...
                        new ClanRelationModule(pChar.Clan.GetRelation(gameSession.Player.Clan)), pChar.Gates.CalculateRings(),
                        false, false, pChar.Invisible, 0, 0, new List<VisualModifierCommand>(),
                        new commandK13(commandK13.DEFAULT)).Bytes;
                }
                ...
                else
                {
                    bytes = commands.new_client.ShipCreateCommand.write(character.Id,
                        ...
                        new ClanRelationModule(character.Clan.GetRelation(gameSession.Player.Clan)),
                        0,
                        false, true, character.Invisible, 0, 0, new List<VisualModifierCommand>(),
                        new commandK13(commandK13.DEFAULT)).Bytes;
                }
            }
            else
        ...
        public void AssetCreateCommand(GameSession gameSession, Asset asset)
        {
            if (gameSession.Player.UsingNewClient)
            {
                gameSession.Client.Send(commands.new_client.AssetCreateCommand.write(
                    new AssetTypeModule((short) asset.Type), asset.Name, (int) asset.Faction,
                    ...
                    new ClanRelationModule(asset.Clan.GetRelation(gameSession.Player.Clan)),
                    new List<VisualModifierCommand>()).Bytes);
            }
            else
            {
        ...
        public void StationCreateCommand(GameSession gameSession, Station station)
        {
            if (gameSession.Player.UsingNewClient)
            {
                ...
                {
                    gameSession.Client.Send(commandY2c.write(station.Id, 6, 1500,
                        station.Position.X, station.Position.Y));
                }
                else if (station is HealthStation)
                {
                    gameSession.Client.Send(commandY2c.write(station.Id, 4, 1500,
                        station.Position.X, station.Position.Y));
                }
                else if (station is ReadyRelayStation)
                {
                    gameSession.Client.Send(commandY2c.write(station.Id, 5, 1500,
                        station.Position.X, station.Position.Y));
                }
                else
                {
            ...
            short effect)
        {
            ...
            {
                if (damage == 0)
                    gameSession.Client.Send(AttackMissedCommand
                        .write(new AttackTypeModule(effect), target.Id, 0).Bytes);
                else
                    gameSession.Client.Send(commands.new_client.AttackHitCommand.write(
                            new AttackTypeModule(effect), attackerId,
            ...
            else
            {
                if (gameSession.Player.UsingNewClient)
                {
                    gameSession.Client.Send(commands.new_client.PetActivationCommand.write(pet.GetOwner().Id, pet.Id,
                        ...
                        new ClanRelationModule(pet.Clan.GetRelation(gameSession.Player.Clan)),
                        pet.Position.X, pet.Position.Y, pet.Speed, false, !pet.GetOwner().Invisible,
                        new commandK13(0)).Bytes);
                }
                else
                {
        ...
        public void CreateBoxCommand(GameSession gameSession, Collectable collectable)
        {
            ...
            if (gameSession.Player.UsingNewClient)
            {
                gameSession.Client.Send(commands.new_client.CreateBoxCommand.write(collectable.Type.ToString(),
                        new BoxModule(collectable.Hash, collectable.Position.X,
        ...
        public void BeaconCommand(GameSession gameSession)
        {
            ...
            {
                LegacyModule(gameSession, "0|" + ServerCommands.BEACON + "|" + player.Position.X + "|" +
        ...
        public void HotkeysCommand(GameSession gameSession)
        {
            ...
            if (newClient)
                gameSession.Client.Send(UserKeyBindingsUpdate.write(keys, false));
        ...
        public void MapAddPOICommand(GameSession gameSession, POI poi)
        {
            if (gameSession.Player.UsingNewClient)
            {
                gameSession.Client.Send(commands.new_client.MapAddPOICommand.write(poi.Id,
                    new POITypeModule((short) poi.Type), poi.TypeSpecification,
                    new POIDesignModule((short) poi.Design), (short) poi.Shape,
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.MapAddPOICommand.write(poi.Id,
                    ...
                    poi.ShapeCordsToInts(), poi.Inverted, poi.Active));
                gameSession.Client.Send(POIReadyCommand.write());
            }
        }
        ...
        public void AddOreCommand(GameSession gameSession, Ore ore)
        {
            if (gameSession.Player.UsingNewClient)
            {
                gameSession.Client.Send(commands.new_client.AddOreCommand
                    .write(new BoxModule(ore.Hash, ore.Position.X, ore.Position.Y),
                        new OreTypeModule((short) ore.Type)).Bytes);
            }
            else
            {
                gameSession.Client.Send(commands.old_client.AddOreCommand.write(ore.Hash,
                    new OreTypeModule((short) ore.Type), ore.Position.X, ore.Position.Y).Bytes);
            }
        }
        ...
        public void PetGearAddCommand(GameSession gameSession, PetGear gear)
        {
            ...
            {
                gameSession.Client.Send(commands.old_client.PetGearAddCommand
                    .write(new PetGearTypeModule((short) gear.TypeOfGear), gear.Level, gear.Amount,
                        gear.Enabled).Bytes);
            }
        }
        ...
        public void PetGearSelectCommand(GameSession gameSession, PetGear gear)
        {
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.PetGearSelectCommand
                    .write(new PetGearTypeModule((short) gear.TypeOfGear), gear.OptionalParams).Bytes);
            }
        }
        ...
        public void AmmunitionCountUpdateCommand(GameSession gameSession, string lootId, int amount)
        {
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.AmmunitionCountUpdateCommand
                    .write(new List<AmmunitionCountModule>()
                    {
                        new AmmunitionCountModule(AmmoConverter.ToAmmoType(lootId), amount)
                    }).Bytes);
            }
        }
        ...
        public void AttributeBoosterUpdateCommand(GameSession gameSession)
        {
            ...
            {
                var boostList = new List<BoosterUpdateModule>();
                if (gameSession.Player.BoostedDamage > 0)
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short) Booster.Types.DAMAGE),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.MAXHP),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.SHIELD),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.QUESTREWARD),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.BONUSBOXES),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.REPAIR),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.HONOUR),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.EP),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.RESOURCE),
                ...
                {
                    boostList.Add(new BoosterUpdateModule(
                        new BoostedAttributeTypeModule((short)Booster.Types.SHIELDRECHARGE),
                ...
                foreach (var booster in pBoosterList)
                {
                    boostList.Find(x => x.attributeType.typeValue == (short)booster.Type).boosterTypes
                        .Add(new BoosterTypeModule((short)booster.BoosterType));
                }
        ...
        public void KillScreenCommand(GameSession gameSession, Killscreen killscreen)
        {
            ...
            {
                gameSession.Client.Send(KillScreenPostCommand.write(killscreen.KillerName,
                    killscreen.KillerLink, killscreen.Alias,
                    new DestructionTypeModule(killscreen.GetDestructionType()),
        ...
        public void KillScreenUpdateCommand(GameSession gameSession,
            List<KillScreenOptionModule> options)
        {
            if (gameSession.Player.UsingNewClient)
            {
                //throw new NotImplementedException();
        ...
        public void MapAssetAddBillboardCommand(GameSession gameSession, Billboard billboard)
        {
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.MapAssetAddBillboardCommand.write("",
                    ...
                    new PartnerTypeModule(billboard.Advertiser), billboard.Position.X,
        ...
        public void BattleStationBuildingStateCommand(GameSession gameSession, Asteroid asteroid)
        {
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.BattleStationBuildingStateCommand.write(asteroid.Id,
                    ...
                    asteroid.Clan.Name, new FactionModule(2)).Bytes);
            }
        }
        ...
        public void AbilityStatusFullCommand(GameSession gameSession, List<Ability> abilities)
        {
            ...
            {
                List<AbilityStatusSingleCommand> abilitiesModule =
                    new List<AbilityStatusSingleCommand>();
                foreach (var ability in abilities)
                {
                    abilitiesModule.Add(
                        new AbilityStatusSingleCommand(ability.AbilityId, ability.Enabled));
                }
        ...
        public void AbilityEffectDeActivation(GameSession gameSession, Ability ability)
        {
            ...
            {
                gameSession.Client.Send(AbilityEffectDeActivationCommand
                    .write(ability.AbilityId, ability.ActivatorId, ability.TargetIds).Bytes);
            }
        }
        ...
        public void MapEventOreCommand(GameSession gameSession, Ore ore, OreCollection eventType)
        {
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.MapEventOreCommand.write((short) eventType,
                    new OreTypeModule((short) ore.Type), ore.Hash).Bytes);
            }
        }
        ...
        public void QuestGiversAvailableCommand(GameSession gameSession, QuestGiver questGiver)
        {
            ...
            else
            {
                gameSession.Client.Send(commands.old_client.QuestGiversAvailableCommand
                    .write(new List<QuestGiverModule>
                    {
                        new QuestGiverModule(questGiver.QuestGiverId, questGiver.Id)
                    }).Bytes);
            }
        }
        ...
        public void QuestListCommand(GameSession gameSession)
        {
            ...
            else
            {
                ...
                var list = new List<QuestSlimInfoModule>();
                foreach (var quest in quests)
                {
                    list.Add(new QuestSlimInfoModule(
                        quest.Id, quest.Root.Id, 0,
                        new List<QuestTypeModule>
                        {
                            new QuestTypeModule((short) quest.QuestType)
                        },
                        new QuestIconModule((short) quest.Icon),
                        new QuestAcceptabilityStatus(
        ...
        public void QuestConditionUpdateCommand(GameSession gameSession, QuestSerializableState state)
        {
            ...
            {
                gameSession.Client.Send(commands.old_client.QuestConditionUpdateCommand.write(state.ConditionId, new QuestConditionStateModule(state.CurrentValue, state.Active, state.Completed)).Bytes);
            }
        }
        #endregion
        ...
        public void PetBuffCommand(GameSession gameSession, bool add, BuffPattern effectId, List<int> addingParameters)
        {
            ...
            {
                gameSession.Client.Send(commands.old_client.PetBuffCommand.write((short)(add ? 0 : 1), (short)effectId, addingParameters).Bytes);
            }
        }
        #endregion
        ...
        public void HellstormAttackCommand(GameSession gameSession, int attackerId, int targetId, bool hit, int currentLoad, short rocketType)
        {
            ...
            {
                gameSession.Client.Send(commands.old_client.HellstormAttackCommand.write(attackerId, targetId, hit, currentLoad, new AmmunitionTypeModule(rocketType)).Bytes);
            }
        }
        #endregion
